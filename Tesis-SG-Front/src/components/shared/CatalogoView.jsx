// src/components/shared/CatalogoView.jsx
...
import { Filter, Plus, ChevronLeft, ChevronRight, FileDown } from "lucide-react";
import {
...
export default function CatalogoView({ titulo, entidad, columnas, data, onEditar, onEliminar }) {
  ...
  const resultadosPaginados = resultados.slice(indiceInicial, indiceInicial + filasPorPagina);
      ...
      <CardContent className="py-6 px-6 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <Button onClick={() => navigate(`/${entidad}/nuevo`)} className="bg-blue-600 hover:bg-blue-700 text-white text-sm px-4 py-2 rounded-lg">
            ...
            <Button variant="outline" className="text-sm bg-white border border-gray-300 shadow-sm" onClick={() => exportToCSV(resultados, columnKeys)}>
              <FileDown size={16} className="mr-2" /> Exportar CSV
            </Button>
            <Button variant="outline" className="text-sm bg-white border border-gray-300 shadow-sm" onClick={() => exportToPDF(resultados, columnKeys)}>
              ...
              {resultadosPaginados.map((item, idx) => (
                <TableRow key={idx} className="hover:bg-gray-50 transition">
                  ...
                  <TableCell className="flex justify-end gap-2 py-3">
                    <Button variant="outline" onClick={() => onEditar(item)} className="text-sm">
                      Editar
                    </Button>
                    <Button
                      size="sm"
